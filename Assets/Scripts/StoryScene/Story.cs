...
public class Story : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string[] bg_paths = new string[9] { "bus stop", "scene1", "Park", "grocery store", "Shopping", "Cinema", "Shopping", "scene1", "Earthquake" };
        //emj_paths = new string[12] { "13.png" , "14.png" , "15.png", "11.png", "12.png", "18.png", "17.png", "19.png", "24.png", "17.png", "22.png", "25.png", };
        emj_paths = new string[12] { "13", "14", "15", "11", "12", "18", "17", "19", "24", "17", "22", "25", };
        //@@子オブジェクト
        ...
        lines.text = " ";
        linestransform = lnObject.GetComponent<RectTransform>();
        ...
        //emj_texture = Resources.Load("Image/StageSelect/OrangeButton", typeof(Texture2D)) as Texture2D;
        
        //emjimage.sprite = Sprite.Create(emj_texture, new Rect(0, 0, emj_texture.width, emj_texture.height), Vector2.zero);
        emjimage.sprite = Resources.Load<Sprite>(iopath+"Emoji/"+emj_paths[scene_index]);
        Debug.Log($"Emoji path is {iopath + "Emoji/" + emj_paths[scene_index]}");
        ...
        chara = ld.Load();
        chara = chara.Substring(0, chara.Length -4);
        Debug.Log($"Chara source is {chara}");
        //Texture2D chara_texture = (Texture2D)AssetDatabase.LoadAssetAtPath<Texture>(ipath + "character/" + chara);
        charaimage.sprite = Resources.Load<Sprite>(iopath + "character/" + chara);
        //Sprite.Create(chara_texture, new Rect(0, 0, chara_texture.width, chara_texture.height), Vector2.zero);
        ...
        bgimage.sprite = Resources.Load<Sprite>(iopath + "Backgrounds/" + bg_paths[scene_index]);
        //Sprite.Create(bg_texture, new Rect(0, 0, bg_texture.width, bg_texture.height), Vector2.zero);
        ...
        lnimage.sprite = Resources.Load<Sprite>(iopath + "UI/Rectangle89");
        //Sprite.Create(ln_texture, new Rect(0, 0, ln_texture.width, ln_texture.height), Vector2.zero);
        //セリフ背景画像の位置配置
    ...
    public IEnumerator startStory()
    {
        ...
        lnimage.sprite = Resources.Load<Sprite>(iopath + "UI/Rectangle888");
        //Sprite.Create(ln_texture, new Rect(0, 0, ln_texture.width, ln_texture.height), Vector2.zero);
    ...
    public void OnNext()
    {
        if (load_flag && start_flag)
        {
            ...
            else //if (nextcount == now_line.Length-1)
            {
                ...
                emjimage.sprite = Resources.Load<Sprite>(iopath + "Emoji/" + emj_paths[scene_index+1]);
                //Sprite.Create(emj_texture, new Rect(0, 0, emj_texture.width, emj_texture.height), Vector2.zero);
                ...
                lnimage.sprite = Resources.Load<Sprite>(iopath + "UI/Rectangle88");
                //Sprite.Create(ln_texture, new Rect(0, 0, ln_texture.width, ln_texture.height), Vector2.zero);
                //セリフ背景画像の位置配置
                lnimagetransform.position = new Vector3(0, -0.4f, 0);
                lnimagetransform.localScale = new Vector3((float)0.35, (float)0.35, (float)1.0);
                //セリフの位置配置
                linestransform.position = new Vector3(0, -1.15f, 0);
    ...
    public void OnBack()
    {
        if (load_flag)
        {
            ...
            else //if (nextcount == now_line.Length-1)
            {
                ...
                emjimage.sprite = Resources.Load<Sprite>(iopath + "Emoji/14");
                Sprite.Create(emj_texture, new Rect(0, 0, emj_texture.width, emj_texture.height), Vector2.zero);
                emjObject.SetActive(true);
                stObject.SetActive(true);
                //ln_texture = (Texture2D)AssetDatabase.LoadAssetAtPath<Texture>(ipath + "UI/Rectangle 88.png");
                ...
                lnimagetransform.localScale = new Vector3((float)0.35, (float)0.35, (float)1.0);
                //セリフの位置配置
                linestransform.position = new Vector3(0, -50, 0);
